...
static LHDropDownControlView *shared = nil;
...
+(instancetype)shared {
    ...
    dispatch_once(&onceToken, ^{
        if (shared == nil) {
            shared = [[LHDropDownControlView alloc]init];
        }
    });
    return shared;
...
- (void)inactivateControl {
    mControlIsActive = NO;
    NSLog(@"%ld",mSelectionIndex);
    if (mSelectionIndex >3 || mSelectionIndex < 0) {
        return;
    }
    [LHDropDownControlView shared].title = mSelectionTitles[mSelectionIndex];
     NSLog(@"%@", self.title);
    NSLog(@"%@",[LHDropDownControlView shared].title);
     [self.delegate dropDownControlView:self didFinishWithSelection:nil];
